...
import { checkVaildationEmail, getWelcomeTemplate, sendtemplateToSms } from "./email.js";
...
app.post('/users',(req,res)=>{
  const user =req.body.myuser
    ...
    const isValid = checkVaildationEmail(user.email);
    if (isValid) {
      // 2. 가입환영 템플릿 만들기
      const template = getWelcomeTemplate(user);
  
      // 3. 이메일에 가입환영 템플릿 전송하기
      sendtemplateToSms(user.email, template);
      res.send("가입완료!!!")
    }
})
